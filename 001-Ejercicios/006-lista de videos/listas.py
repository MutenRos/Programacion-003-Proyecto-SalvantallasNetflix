...
def main():
    ...
    # Mejora: resumen final con estadisticas del scraping
    total_videos = sum(p.get("videos_count", 0) for p in output["playlists"])
    total_playlists = len(output["playlists"])
    total_errors = sum(1 for p in output["playlists"] if "error" in p)

    print(f"\n{'='*40}")
    print(f"  RESUMEN DEL SCRAPING")
    print(f"{'='*40}")
    print(f"  Playlists encontradas: {total_playlists}")
    print(f"  Videos totales:        {total_videos}")
    if total_errors > 0:
        print(f"  Playlists con error:   {total_errors}")
    print(f"  JSON guardado en:      {OUT_JSON}")
    print(f"  Thumbnails videos:     {THUMBS_DIR}/")
    print(f"  Thumbnails playlists:  {PLAYLIST_THUMBS_DIR}/")
    print(f"{'='*40}")
